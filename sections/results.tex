...
% the rollout phase, the agent will use this action to simulate a possible trajectory starting from a sampled belief state. At the node representing the chosen action and its corresponding observation, the simulated next state will be added to the belief. 

% resulting 

% add a new node for the action and the corresponding observation after simulating a possible trajectory starting with this action (see Section \section{pomcp}).



% The advantage of the agent using preferred actions becomes clearer than before during this experiment. 

% The agent with the full action space initially values every action equally. Actions with a low payoff are incrementally selected less by the UCB1 action selection (see Section \ref{sec:pomcp}). However, some exploration is necessary. Therefore, many potentially unlikely extreme scenarios are simulated during planning. Thereby, the amount of searches with more likely high-rewarding minor actions is reduced. Particle deprivation can occur. Particle injection helps but is no definite cure. Because of this, this agent needs many searches to be able to account for overcorrection when a distracted driver becomes attentive again. At 750 searches, the agent fails to do so. 34 of the terminal states occur when the driver is attentive. In the other cases, the value of extreme actions is overestimated, which leads to extreme actions in the real environment and results in driving off the track. % The end terminal state is less important than when the agent started cheating

% Mention how many times the agents lose track after the driver oversteering. And that they use random action selection for some time after
...
% The overcorrection is relative to the amount the driver needs to correct the steering. 
% Preferred gets a better result and can keep


% The reduced action space for the agent with an action space subset is intended to mitigate this. The need for extreme actions is scarce. Only evaluating minor actions 


% However, whenever a distracted driver regains attentiveness, and overcorrects her steering, the agent may be required to counteract with a sufficiently strong steering action. When this happens 
...
% Due to the overcorrection, 


% having a particle in the belief that represents a driver who becomes attentive again in the the exact moment the real driver regains its attentiveness


% a driver  is more important as  
