\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Performance comparison of POMCP when utilizing the All actions, a Bounded actions, or preferred actions with a driver model that over-corrects when it regains attention and performs noisy actions. Each point shows the mean cumulative reward from 50 runs with 1000 actions each, if no terminal state is reached earlier.}
\end{figure}