\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption[Performance of POMCP with a simple driver model]{Performance comparison of POMCP when utilizing all actions, an action subset, or preferred actions with a simple driver model. Each point shows the mean cumulative reward from 50 runs with 1,000 actions each, if no terminal state is reached earlier.}
    \label{fig:conv_norm}
\end{figure}